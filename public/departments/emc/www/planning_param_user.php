...
// Filtre Groupe Lieu
if(!isset($_SESSION['filtreGroupeLieu1'])) {
	$_SESSION['filtreGroupeLieu1'] = array();
}
$smarty->assign('filtreGroupeLieu1', $_SESSION['filtreGroupeLieu1']);
...
//Test
$lieuxs = new GCollection('Lieu');		
$sql = "SELECT planning_lieu.*, planning_user.couleur as color		
        FROM planning_lieu
		INNER JOIN planning_user on planning_user.user_id = planning_lieu.lieu_id";		
// Si filtre sur groupe lieu		
if(count($_SESSION['filtreGroupeLieu2']) > 0) {		
    $sql.= " WHERE lieu_id IN ('" . implode("','", $_SESSION['filtreGroupeLieu1']) . "')";		
}		
$sql.= " ORDER BY nom ASC";
$lieuxs->db_loadSQL($sql);  
