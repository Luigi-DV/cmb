...
if(count($_SESSION['filtreGroupeLieu']) > 0) {
	...
	$mysqli = new mysqli("localhost", "root",'', "clima");
...
if ($base_colonne=='heures')
{
	...
	while ($tmpDate <= $dateFin) 
	{
		...
		foreach ($tabTranchesHoraires as $trancheHeureCourante) 
		{
			if ($trancheHeureCourante == 6)
			{	
			$trancheFin = 14;
			}
			elseif ($trancheHeureCourante == 14)
			{$trancheFin = 22;
			}
			else {$trancheHeureCourante = "";
				$trancheFin = "";
			}
			if ($trancheHeureCourante == 6){
			$html .= '<th class="planning_head_hour ' . $sClass . '" style="width:' . $largeurCase . 'px;">' . "AM" . '</th>';}
			elseif ($trancheHeureCourante == 14)
			{
			$html .= '<th class="planning_head_hour ' . $sClass . '" style="width:' . $largeurCase . 'px;">' . "PM" . '</th>';}
			else {$html .= '<th class="planning_head_hour ' . $sClass . '" style="width:' . $largeurCase . 'px;">'   . "" . '</th>';}
		}
		$tmpDate->modify('+1 day');
	}
	$html .= '</tr></thead><tbody>' . CRLF;
