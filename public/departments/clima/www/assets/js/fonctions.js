...
function videChampsFinTache(actif) {
	...
	champs['allday'] = 'checkbox';
	champs['matin'] = 'checkbox';
	champs['apresmidi'] = 'checkbox';
	champs['nuit'] = 'checkbox';
	champs['holiday'] = 'checkbox';
...
function cellClic2(id,type)
{
	...
	if (type==1)
	{
		...
		if (idtab[3] == null)
		{
			xajax_ajoutHoliday(datedebut, projet);
		}else xajax_ajoutHoliday(datedebut, projet,'',idtab[3]);
	}else
	{
		var idtab=id.split('_');
		xajax_modifHoliday(idtab[1]);
	}
	return false;
}
...
function calculateDateFin(initialDate) {
	var days = document.getElementById("days").value;
	var sDate = initialDate.split("/");
	var fdays = Number(sDate[0]) + Number(days);
	var fmonth = Number(sDate[1]);
	var fyear = Number(sDate[2]);
	while((fdays > 28 && fmonth == 2) || (fdays > 31 && (fmonth == 1 || fmonth == 3 || fmonth == 5 || fmonth == 7 || fmonth == 8 || fmonth == 10 || fmonth == 12)) || (fdays > 30 && (fmonth == 4 || fmonth == 6 || fmonth == 9 || fmonth == 11)))
	{
		...
		{
			if(fdays >= 29 && (((fyear % 4 == 0) && (fyear % 100 != 0)) || (fyear % 400 == 0)) ){ // Bisiesto
				fmonth = fmonth + 1;
				fdays = fdays - 29;
			}
			else{
	...
	if(fdays<10){
		if(fmonth<10){
			document.getElementById('dateFinRepetitionJour').value = "0" + fdays + "/" + "0" + fmonth + "/" + fyear;
			document.getElementById('periode_derniere').value = "0" + fdays + "/" + "0" + fmonth + "/" + fyear;
		}
		else{
			document.getElementById('dateFinRepetitionJour').value = "0" + fdays + "/" + fmonth + "/" + fyear;
			document.getElementById('periode_derniere').value = "0" + fdays + "/" + fmonth + "/" + fyear;
		}
	}
	else{
		if(fmonth<10){
			document.getElementById('dateFinRepetitionJour').value = fdays + "/" + "0" + fmonth + "/" + fyear;
			document.getElementById('periode_derniere').value = fdays + "/" + "0" + fmonth + "/" + fyear;
		}
		else{
			document.getElementById('dateFinRepetitionJour').value = fdays + "/" + fmonth + "/" + fyear;
			document.getElementById('periode_derniere').value = fdays + "/" + fmonth + "/" + fyear;
		}
	}
}

function calculateDays(initialDate,finalDate) {
	var aFecha1 = initialDate.split('/');
	var aFecha2 = finalDate.split('/');
