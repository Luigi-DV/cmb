	<div class="row noprint">
		<div class="col-md-12 mb-2" id="firstLayer">
			<div class="soplanning-box form-inline pt-1" id="divPlanningDateSelector">
				<div class="hidden btn-group cursor-pointer pt-2" id="btnDateNow">
					<a class="btn btn-default tooltipster" title="{#aujourdhui#}{$dateToday}" onClick="document.location='process/planning_a.php?raccourci_date=aujourdhui'" id="buttonDateNowSelector"><i class="fa fa-home fa-lg fa-fw" aria-hidden="true"></i></a>
				</div>
				{* DIV POUR CHOIX DATE *}
					<div class="btn-group ml-md-2 pt-2" id="dropdownDateSelector">
						<form action="process/planning_a.php" method="GET" class="form-inline" id="formChoixDates">
				...
				<div class="btn-group ml-md-2 pt-2 cursor-pointer" id="btnDateSelector">
					<a class="btn btn-default" onClick="document.location='process/planning_a.php?raccourci_date=-7';" id="buttonDatePrevSelector"><i class="fa fa-chevron-left fa-lg fa-fw" aria-hidden="true"></i></a>
					<a class="btn btn-default" onClick="document.location='process/planning_a.php?raccourci_date=+7';" id="buttonDateNextSelector"><i class="fa fa-chevron-right fa-lg fa-fw" aria-hidden="true"></i></a>
				</div>
				...
				{* DIV POUR CHOIX FILTRE PROJETS *}
					<div class="btn-group pt-2" id="dropdownTaskProjectFilter">
						<form action="process/planning_a.php" method="POST">
					...
					<div class="btn-group pt-2" id="dropdownAdvancedFilter">
						...
						<ul class="dropdown-menu">
							{if (($filtreStatutTache|@count > 0) or ($filtreGroupeLieu|@count >0) or ($filtreGroupeRessource|@count >0) or ($filtreStatutProjet|@count >0)  )}<a href="process/planning_a.php?desactiverFiltreAvances=1" class="btn btn-danger btn-sm margin-left-10">{#formFiltreAvancesDesactiver#}</a>{/if}
							<li class="divider"></li>
							<li>
					...
					<div class="btn-group pt-2" id="dropdownTypePlanning">
						<button class="btn dropdown-toggle btn-default" data-toggle="dropdown" onclick="javascript:multiselecthide();"><i class="fa fa-calendar fa-lg fa-fw" aria-hidden="true"></i><span id='label_tierpar'></span>&nbsp;<span class="caret"></span></button>
						<div class="dropdown-menu">
							{if $smarty.session.baseLigne eq 'users'}
								<a class="dropdown-item" href="process/planning_a.php?baseLigne=users">
							...
							{if $smarty.session.baseLigne eq 'projets'}
								<a class="dropdown-item" href="process/planning_a.php?baseLigne=projets">
								<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
							{else}
								<a class="dropdown-item" href="process/planning_a.php?baseLigne=projets">
								<i style="margin-left:19px;">&nbsp;</i>
							...
							{if $smarty.session.masquerLigneVide eq 0 }
								<a class="dropdown-item" href="process/planning_a.php?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&masquerLigneVide=1">
								<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
							{else}
								<a class="dropdown-item" href="process/planning_a.php?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&masquerLigneVide=0">
								<i style="margin-left:19px;">&nbsp;</i>
							{/if}
							{#planningAfficherLignesVides#}</a>

							{if $smarty.session.afficherTableauRecap eq 1}
								<a class="dropdown-item" href="process/planning_a.php?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherTableauRecap=0">
								<i class="fa fa-check" aria-hidden="true"></i>&nbsp;&nbsp;
							{else}
								<a class="dropdown-item" href="process/planning_a.php?baseLigne={$smarty.session.baseLigne}&baseColonne={$smarty.session.baseColonne}&afficherTableauRecap=1">
								<i style="margin-left:19px;">&nbsp;</i>
							{/if}
					...
					<div class="btn-group pt-2" id="dropdownLarge">
						{if $dimensionCase eq "reduit"}
							<a class="btn btn-default" title="{#menuPlanningLarge#}" href="process/planning_a.php?dimensionCase=large&date_debut_affiche={$dateDebut}&date_fin_affiche={$dateFin}&date_fin_custom=2_semaines"><i class="fa fa-search-plus fa-lg fa-fw" aria-hidden="true"></i></a>
						{else}
							<a class="btn btn-default" title="{#menuPlanningReduit#}" href="process/planning_a.php?dimensionCase=reduit&date_debut_affiche={$dateDebut}&date_fin_affiche={$dateFin}&date_fin_custom=3_mois"><i class="fa fa-search-minus fa-lg fa-fw" aria-hidden="true"></i></a>
						{/if}
					</div>
								...
								<div class="input-group-append">
									...
									{if $filtreTexte != ""}
										<div class="btn-group">
											...
											<ul class="dropdown-menu">
												<li><a href="process/planning_a.php?desactiverFiltreTexte=1">{#formFiltreUserDesactiver#}</a></li>
											</ul>
										</div>
									{/if}
								</div>
				...
				{* DIV POUR MODIFIER ET SUPPRIMER TACHES *}
					...
					<div class="pt-1 " ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<img src="{$BASE}/luna.png" height="30px" id="night" name="night"/>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
