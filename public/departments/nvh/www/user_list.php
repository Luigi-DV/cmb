...
if (isset($_GET['order']) && in_array($_GET['order'], array('nom', 'user_id', 'nom_groupe'))) {
	$order = $_GET['order'];
...
$sql = 'SELECT distinct pu.nom, pu.email, pu.user_id, pu.login, pu.visible_planning, pu.order_planning, pu.couleur, pu.droits, pug.nom AS nom_groupe, pu.user_groupe_id, pu.adresse, pu.telephone, pu.mobile, pu.metier, pu.commentaire, pu.date_dernier_login, pu.login_actif, COUNT(pp.periode_id) AS totalPeriodes
...
else{$sql .= "		AND pug.type_groupe = 'user' ";}
...
$equipes->db_load(array('type_groupe', '=', 'user'), array('nom' => 'ASC'));
