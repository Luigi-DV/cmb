<?php
	...
	foreach ($planning['lignes'] as $cle => $infos)
	{
			...
			foreach ($planning['taches'][$cle] as $periodes)
			{
				foreach ($periodes as $p)
				{
					...
					if (is_null($infos_tache['date_fin'])) {
						...
						$html_recap .= sqldate2userdate($infos_tache['date_fin']);
					}
					// Lieu
					if (!is_null($infos_tache['lieu'])) {
						$html_recap .= "<br /><i class='fa fa-user' aria-hidden='true'></i>&nbsp;&nbsp;".$infos_tache['lieu_nom'];
					}
					// Ressource
					...
					if(!is_null($infos_tache['date_fin'])) 
					{
						...
						while ($date1 <= $date2) 
						{
							...
							if (!in_array($date1->format('Y-m-d'), $joursFeries)) 
							{
								$totalJours +=1;
								if($date1 < $now) {
									$totalJoursPassed +=1;
		...
		if ($infos_tache['duree_details'] != 'duree'){
		$TotalMaxJour = 8;}
