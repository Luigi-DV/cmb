...
<form class="form-horizontal" method="POST" action="" target="_blank">
	...
	<div class="form-group row col-md-12">
		...
		<div class="col-md-7">
			...
			{if $audit.type=="SA"}{#action_ajout_sample#}"{$audit.informations}"{/if}		
			{if $audit.type=="SM"}{#action_modif_sample#} "{$audit.informations}"{/if}
			{if $audit.type=="SD"}{#action_suppression_sample#} "{$audit.informations}"{/if}
			</p>
		</div>
	</div>	
