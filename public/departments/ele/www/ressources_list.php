...
if (isset($_GET['order']) && in_array($_GET['order'], array('nom', 'email', 'user_id', 'login', 'visible_planning', 'order_planning', 'nom_groupe'))) {
	//$order = $_GET['order'];
	$order = 'order_planning';
...
$sql = 'SELECT distinct pu.nom, pu.email, pu.user_id, pu.login, pu.visible_planning, pu.order_planning AS order_planning, pu.couleur, pu.droits, pug.nom AS nom_groupe, pu.adresse, pu.telephone, pu.mobile, pu.metier, pu.commentaire, pu.date_dernier_login, pu.login_actif, COUNT(pp.periode_id) AS totalPeriodes
...
else{$sql .= "		AND pug.type_groupe = 'ressource' ";}
...
$equipes->db_load(array('type_groupe', '=', 'ressource'), array('nom' => 'ASC'));
$smarty->assign('equipes', $equipes->getSmartyData());


$smarty->assign('xajax', $xajax->getJavascript("", "assets/js/xajax.js"));

$smarty->display('www_ressources_list.tpl');
?>
