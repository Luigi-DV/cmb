...
require(BASE . '/planning_param_equi.php');
...
$sql = "SELECT planning_periode.*,planning_projet.statut, planning_status.nom as status_nom,  planning_status.barre as statut_barre,planning_status.gras as statut_gras,planning_status.italique as statut_italique,planning_status.souligne as statut_souligne, planning_status.couleur as statut_couleur,planning_status.pourcentage as statut_pourcentage, pu.nom as user_nom, pu.couleur as user_couleur,
		...
		pl.nom as lieu_nom, planning_projet.charge as charge, planning_projet.createur_id AS projet_createur_id,
...
if(count($_SESSION['filtreGroupeRess1']) > 0) {
	$sql .= "AND ( planning_periode.ressource_id LIKE " . val2sql('%' . $_SESSION['filtreGroupeRess1'][0] . '%');
    foreach($_SESSION['filtreGroupeRess1'] as $res) {
...
while ($p = $ressourcxs->fetch()) {
		$infosJour = $p->getSmartyData();
		// On force les valeurs nulles
		$planning['lignes'][$infosJour['ressource_id']]=array('id'=>$infosJour['ressource_id'],'nom'=>$infosJour['nom'],'couleur'=>null,'url_modif'=>null);
}
...
while ($p = $periodes->fetch()) {
	...
	//TEST
	$res_plan = explode(',',$infosJour['ressource_id']);
	foreach($res_plan as $replan){
		if (!in_array($replan,$planning['ressources']))
		{
			$planning['ressources'][]=$replan;
		}
	}
	...
	// traitement de chaque jour (construction du planning en mode jours)
		...
		while ($tmpDate <= $dateFin_planning) {
			...
			foreach(explode(',',$infosJour['ressource_id']) as $resss) {
				$planning['taches'][$resss][$cle][]=$infosJour['periode_id'];
			}
...
// Colonnes jour
	...
	while ($tmpDate <= $dateFin) {
		...
		$headerNomJours .= '<th class="planning_head_dayname ' . $sClass . '"><div><a href="process/planning_equi.php?date_debut_affiche='.$tmpJourDateDebut.'&date_fin_affiche='.$tmpJourDateFin.'">' . strtoupper(substr($smarty->getConfigVars('day_' . $tmpDate->format('w')), 0, 1)) . '</a></div></th>' . CRLF;
		$headerNumeroJours .= '<th class="planning_head_day ' . $sClass . '"><a href="process/planning_equi.php?date_debut_affiche='.$tmpJourDateDebut.'&date_fin_affiche='.$tmpJourDateFin.'">' . $tmpDate->format('j') . '</a></th>' . CRLF;
		$nomMoisCourant = $smarty->getConfigVars('month_' . $tmpDate->format('n'));
		if ($nomMoisCourant . ' ' . $tmpDate->format('Y') == $tmpMois) {
			$colspanMois++;
		} else {
			$headerMois .= '<th class="planning_head_month" colspan="' . $colspanMois . '"><a href="process/planning_equi.php?date_debut_affiche='.$tmpMoisDateDebut.'&date_fin_affiche='.$tmpMoisDateFin.'">' . $tmpMois . '</a></th>' . CRLF;
			$colspanMois = '1';
		...
		if ($tmpDate->format('w') == 0) {
			...
			$headerSemaines .= '<th class="planning_head_week" colspan="' . $colspanSemaine . '"><a href="process/planning_equi.php?date_debut_affiche='.$tmpSemaineDateDebut.'&date_fin_affiche='.$tmpSemaineDateFin.'">' . $smarty->getConfigVars('planning_semaine') . ' ' . $tmpDate->format('W') . '</a></th>' . CRLF;
			$colspanSemaine = 1;
		} else {
			$colspanSemaine++;
		}
	...
	if($colspanSemaine != 1) {
		...
		$tmp2Date->modify('+' . $nbJours . 'days');
		$tmpSemaineDateFin = $tmp2Date->format(CONFIG_DATE_LONG);
		$headerSemaines .= '<th class="planning_head_week" colspan="' . ($colspanSemaine-1) . '"><a href="process/planning_equi.php?date_debut_affiche='.$tmpSemaineDateDebut.'&date_fin_affiche='.$tmpSemaineDateFin.'">' . $smarty->getConfigVars('planning_semaine') .	' ' . $tmpDate->format('W') . '</a></th>' . CRLF;
	}
...
$smarty->display('www_planning_equi.tpl');