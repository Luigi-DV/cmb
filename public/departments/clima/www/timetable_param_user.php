...
if ($base_ligne=="users")
{
	$linkswitch="process/timetable_user.php?baseLigne=projets&baseColonne=$base_colonne";
}else $linkswitch="process/timetable_user.php?baseLigne=users";
// Autres variables
...
// Filtre Groupe Lieu
if(!isset($_SESSION['filtreGroupeLieu2'])) {
	$_SESSION['filtreGroupeLieu2'] = array();
}
$smarty->assign('filtreGroupeLieu2', $_SESSION['filtreGroupeLieu2']);
...
if((isset($_COOKIE['triPlanningUser']) && (in_array($_COOKIE['triPlanningUser'], $triPlanningPossibleUser) || in_array($_COOKIE['triPlanningUser'], $triPlanningPossibleProjet)))) {
	$_SESSION['triPlanningUser'] = 'user_id asc';
}
...
if(count($_SESSION['filtreGroupeLieu2']) > 0) {		
    $sql.= " WHERE lieu_id IN ('" . implode("','", $_SESSION['filtreGroupeLieu2']) . "')";		
}		
