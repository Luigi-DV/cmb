...
//filtre Scheduled
if (isset($_REQUEST['notScheduled'])) {
	$notScheduled = $_REQUEST['notScheduled'];
} else {
	$notScheduled = 'false';
}
$_SESSION['notScheduled'] = $notScheduled;

if (isset($_REQUEST['order']) && in_array($_REQUEST['order'], array('nom_createur', 'nom', 'charge', 'livraison', 'reception', 'nom_pm', 'price'))) {
...
$smarty->assign('notScheduled', $notScheduled);
...
if(isset($_REQUEST['rechercheProjet']) && $_REQUEST['rechercheProjet'] != ''){
	...
	$sql = "SELECT planning_projet.*, planning_groupe.nom AS nom_groupe, planning_user.nom AS nom_createur, pu.nom AS nom_pm, psb.nom as statut_nom, psb.pourcentage as statut_pourcentage, psb.couleur as statut_couleur, ps.nom as statut_nom_
			FROM planning_projet
			INNER JOIN planning_status psb ON psb.status_id = planning_projet.statut_bill
			...
			LEFT JOIN planning_user pu ON pu.user_id = planning_projet.pm_id
			WHERE (" . $isLike . ") AND planning_projet.statut in ('" . implode("','", $listeStatuts) . "') AND planning_projet.nom != 'timetable' AND planning_projet.nom != 'holiday'";
	if($notScheduled == 'true') {
		$sql .= "AND planning_projet.projet_id NOT IN (SELECT distinct projet_id FROM planning_periode)";
	}
	...
	$sql = "SELECT distinct planning_projet.*, planning_groupe.nom AS nom_groupe, planning_user.nom AS nom_createur, pu.nom AS nom_pm, psb.nom as statut_nom, psb.pourcentage as statut_pourcentage, psb.couleur as statut_couleur, ps.nom as statut_nom_
			FROM planning_projet
			INNER JOIN planning_status psb ON psb.status_id = planning_projet.statut_bill
			INNER JOIN planning_status ps ON ps.status_id = planning_projet.statut
			LEFT JOIN planning_groupe ON planning_groupe.groupe_id = planning_projet.groupe_id
	...
	$sql .= " WHERE planning_projet.statut in ('" . implode("','", $listeStatuts) . "') AND planning_projet.nom != 'timetable' AND planning_projet.nom != 'holiday'";
	if($notScheduled == 'true') {
		$sql .= "AND planning_projet.projet_id NOT IN (SELECT distinct projet_id FROM planning_periode)";
	}
...
//Pending Scheduling 
$projets_scheduled = new GCollection('Projet');
$sql = "SELECT DISTINCT C.* FROM planning_projet C, planning_periode O WHERE C.projet_id = O.projet_id";
$projets_scheduled->db_loadSQL($sql);
$smarty->assign('projets_scheduled', $projets_scheduled->getSmartyData());
