...
//$test=$_SESSION['stats_users']['users'];
$test = array($_REQUEST['user_graph'],0);
...
if(count($test) > 0) {
	$sql .= " AND user_id IN ('" . implode("','", $test) . "')";
}
$sql .= " ORDER BY nom";
...
$sql = "SELECT ppe.*
		...
		";
if(count($test) > 0) {
	$sql .= " AND ppe.user_id IN ('" . implode("','", $test) . "')";
}
...
while ($periode = $periodes->fetch()) {
	...
	if(!is_null($periode->date_fin)) {
		while($tmpDateTache <= $finTache) {
			if( in_array($tmpDateTache->format('Y-m-d'), $joursFeries) || $tmpDateTache > $dateFinGraphe || $tmpDateTache < $dateDebutGraphe) {
	...
	foreach ($usersTab as $user_id) {
		if(isset($valeurs[$user_id])) {
			if($_SESSION['stats_users']['abscisse_echelle_valeur'] == 'heures') {
				$donneesOrdonnee[$user_id][] = 100*convertHourToDecimal($valeurs[$user_id]) / CONFIG_DURATION_DAY;
			} else {
				$donneesOrdonnee[$user_id][] = 100*convertHourToDecimal($valeurs[$user_id]) / CONFIG_DURATION_DAY;
			}
		} else {
