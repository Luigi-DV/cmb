...
// get Select Two values, return select value or array of values if multiple select
function getTwoSelectValues(selectId1, selectId2)
{
	var elmt = document.getElementById(selectId1);
	if(elmt.multiple == false)
	{
		return elmt.options[elmt.selectedIndex].value;
	}
	...
	var elmt = document.getElementById(selectId2);
	if(elmt.multiple == false)
	{
		return elmt.options[elmt.selectedIndex].value;
	}
	for(var i=values.length; i< elmt.options.length; i++)
	{
		if(elmt.options[i].selected == true)
		{
			values[values.length] = elmt.options[i].value;
