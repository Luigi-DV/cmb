...
if ($base_ligne=="users")
{
	$linkswitch="process/planning_equi.php?baseLigne=projets&baseColonne=$base_colonne";
}else $linkswitch="process/planning_equi.php?baseLigne=users";
// Autres variables
...
if(!isset($_SESSION['filtreGroupeRess1'])) {
	$_SESSION['filtreGroupeRess1'] = array();
}
$smarty->assign('filtreGroupeRess1', $_SESSION['filtreGroupeRess1']);
...
if((isset($_COOKIE['triPlanningUser']) && (in_array($_COOKIE['triPlanningUser'], $triPlanningPossibleUser) || in_array($_COOKIE['triPlanningUser'], $triPlanningPossibleProjet)))) {
	$_SESSION['triPlanningUser'] = 'team_nom desc, user_id desc';
}
...
if (CONFIG_SOPLANNING_OPTION_RESSOURCES == 1)
{
	...
	$sql .= " LEFT JOIN planning_ressource_groupe prg ON pr.ressource_groupe_id = prg.ressource_groupe_id";
...
//Test
$ressourcxs = new GCollection('Ressource');
$sql = "SELECT *
		FROM planning_ressource";
// Si filtre sur groupe lieu
if(count($_SESSION['filtreGroupeRess1']) > 0) {
	$sql.= " WHERE ressource_id IN ('" . implode("','", $_SESSION['filtreGroupeRess1']) . "')";
}
$sql.= " ORDER BY nom ASC";
$ressourcxs->db_loadSQL($sql);		
