...
if ($base_colonne=='jours')
{
	...
	while ($tmpDate <= $dateFin) {
		...
		$headerNomJours .= '<th class="planning_head_dayname ' . $sClass . '"><div><a href="process/planning_a.php?date_debut_affiche='.$tmpJourDateDebut.'&date_fin_affiche='.$tmpJourDateFin.'">' . strtoupper(substr($smarty->getConfigVars('day_' . $tmpDate->format('w')), 0, 1)) . '</a></div></th>' . CRLF;
		$headerNumeroJours .= '<th class="planning_head_day ' . $sClass . '"><a href="process/planning_a.php?date_debut_affiche='.$tmpJourDateDebut.'&date_fin_affiche='.$tmpJourDateFin.'">' . $tmpDate->format('j') . '</a></th>' . CRLF;
		$nomMoisCourant = $smarty->getConfigVars('month_' . $tmpDate->format('n'));
		if ($nomMoisCourant . ' ' . $tmpDate->format('Y') == $tmpMois) {
			$colspanMois++;
		} else {
			$headerMois .= '<th class="planning_head_month" colspan="' . $colspanMois . '"><a href="process/planning_a.php?date_debut_affiche='.$tmpMoisDateDebut.'&date_fin_affiche='.$tmpMoisDateFin.'">' . $tmpMois . '</a></th>' . CRLF;
			$colspanMois = '1';
		...
		if ($tmpDate->format('w') == 0) {
			...
			$headerSemaines .= '<th class="planning_head_week" colspan="' . $colspanSemaine . '"><a href="process/planning_a.php?date_debut_affiche='.$tmpSemaineDateDebut.'&date_fin_affiche='.$tmpSemaineDateFin.'">' . $smarty->getConfigVars('planning_semaine') . ' ' . $tmpDate->format('W') . '</a></th>' . CRLF;
			$colspanSemaine = 1;
		} else {
			$colspanSemaine++;
		}
	...
	if($colspanSemaine != 1) {
		...
		$tmp2Date->modify('+' . $nbJours . 'days');
		$tmpSemaineDateFin = $tmp2Date->format(CONFIG_DATE_LONG);
		$headerSemaines .= '<th class="planning_head_week" colspan="' . ($colspanSemaine-1) . '"><a href="process/planning_a.php?date_debut_affiche='.$tmpSemaineDateDebut.'&date_fin_affiche='.$tmpSemaineDateFin.'">' . $smarty->getConfigVars('planning_semaine') .	' ' . $tmpDate->format('W') . '</a></th>' . CRLF;
	}
		...
		foreach ($planning['colonnes'] as $cle_colonne) 
		{
			...
			if ($base_colonne=="jours"||$base_colonne=="users")
			{
				...
				if (isset($joursOccupes[$current_date])) 
				{
					...
					foreach ($joursOccupes[$current_date] as $j) 
					{
						...
						if(in_array($jour['periode_id'], $ordreJourPrec) && $niveauCourant != array_search($jour['periode_id'], $ordreJourPrec)) 
						{
							...
							for($i=1; $i<=$nbVides; $i++) 
							{
								$html_aux .= '<div class="cellProject cellEmpty"></div>' . CRLF;
								$niveauCourant++;
							}
							$niveauCourant++;
					...
					if(strpos($html_aux, "cellProjectAM") && !strpos($html_aux, "cellProjectPM") && !strpos($html_aux, "cellProjectN") && !strpos($html_aux, "cellProject ") && !strpos($html_aux, "cellProjectcons ")) {
						$html.= $html_aux;
						...
						if($dimensionCase == 'large') $html.=createCellTimetable($ligne['id'],$current_date,'PM');
						$html.='</div>';
						$html.= '<div class="cellLine"></div>';
						...
						if($dimensionCase == 'large') $html.=createCellTimetable($ligne['id'],$current_date,'N');
						$html.='</div>';
					} 
					
					//solo tareas de tardes
					elseif (!strpos($html_aux, "cellProjectAM") && strpos($html_aux, "cellProjectPM") && !strpos($html_aux, "cellProjectN") && !strpos($html_aux, "cellProject ") && !strpos($html_aux, "cellProjectcons ")) {
						...
						if($dimensionCase == 'large') $html.=createCellTimetable($ligne['id'],$current_date,'AM');
						$html.='</div>';
						$html.= '<div class="cellLine"></div>';
					...
					elseif (strpos($html_aux, "cellProjectAM") && !strpos($html_aux, "cellProjectPM") && strpos($html_aux, "cellProjectN") && !strpos($html_aux, "cellProject ") && !strpos($html_aux, "cellProjectcons ")) {
						$html_aux= str_replace('<div id="td_' . $ligneId . '_' . $current_date2 . '_N','<div id="td_' . $ligneId . '_' . $current_date2 . '_PM" '. $styleTD. ' class="cellTask' . ($dimensionCase == 'large' ? '' : 'cons') . '" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);">' . ($dimensionCase == 'large' ? createCellTimetable($ligne['id'],$current_date,'PM') : '') . '</div><div id="td_' . $ligneId . '_' . $current_date2 . '_N',$html_aux);
						$html.= str_replace("cellProjectN","cellProjectAM",$html_aux);
					} 
				...
				} else 
				{
					...
					$html.='</div>';
					$html.= '<div class="cellLine"></div>';
					$html.= '<div id="td_' . $ligneId . '_' . $current_date2 . ' '. $styleTD. ' class="cellEmpty" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);"></div>';
					$html .= '</td>' . CRLF;
				}
			}
		}
...
$smarty->display('www_planning_a.tpl');