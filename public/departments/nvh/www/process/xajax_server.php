...
function rejectProjet($projet_id, $iteration) {
	...
	$sql2 = "INSERT INTO `planning_received`(`projet_id`, `NVH`, `NVH_iteration`) VALUES ('".$projet_id."', 'true', '".$iteration."') ON DUPLICATE KEY UPDATE `projet_id`='".$projet_id."', `NVH`='true', `NVH_iteration`='".$iteration."'";
...
function ajoutPeriode($dateDebut = '', $ligne_id = '', $periode_id = '', $heureDebut = '') {
	...
	if (CONFIG_SOPLANNING_OPTION_RESSOURCES == 1)
	{
		...
		WHERE pr.ressource_groupe_id='1' AND date_calibrated>='".date('Y-m-d')."'";
	...
	if ($heureDebut != '') {
		$periode->duree = usertime2sqltime('8:00:00');
...
function ajoutHoliday($dateDebut = '', $ligne_id = '', $periode_id = '', $heureDebut = '') {
	...
	if($user->checkDroit('tasks_modify_all') || $user->checkDroit('tasks_modify_own_project') || $user->checkDroit('tasks_modify_own_task')) {
		...
		$sql .= "	WHERE pu.visible_planning = 'non' AND pu.user_groupe_id != ''";
		if ($user->checkDroit('tasks_view_only_own')) {
			$sql .= " AND pu.user_id = " . val2sql($user->user_id);
		}
		$sql .=	" ORDER BY groupe_nom, pu.nom";
...
function modifPeriode($periode_id) {
	...
	if (CONFIG_SOPLANNING_OPTION_RESSOURCES == 1)
	{
		...
		FROM planning_ressource pr
		LEFT JOIN planning_ressource_groupe prg ON pr.ressource_groupe_id = prg.ressource_groupe_id 
		WHERE date_calibrated>='".date('Y-m-d')."'";
		$sql .=	" ORDER BY groupe_nom, pr.nom";
		$listeRessources->db_loadSQL($sql);
		$smarty->assign('listeRessources', $listeRessources->getSmartyData());
	}
...
function checkHoliday($lieu_id, $date_debut, $matin, $apresmidi, $nuit, $date_fin, $periode_id) {
	...
	foreach( $lieu_id as $lieu){
		$usernom = new Lieu();
		$sql2 = 'SELECT nom FROM planning_lieu WHERE lieu_id = '.val2sql($lieu);
		...
		for($i = 1; $i <= $ndays; $i++){
			...
			$periodeTest = new Periode();
			$sql = 'SELECT * FROM planning_periode WHERE lieu_id = ' . val2sql($lieu) . ' AND projet_id = "holiday" AND date_debut = ' . val2sql($date_aux) . 'AND duree_details IN (' . val2sql($details1) . ','. val2sql($details2) .','. val2sql($details3) . ','. val2sql($details4) . ')';
			if($periode_id != '') {
				$sql .= ' AND periode_id <> ' . val2sql($periode_id);
			}

			if($periodeTest->db_loadSQL($sql)) {
				$objResponse->addAssign('divStatutCheckHoliday'.$j, 'innerHTML', '<font color="#FF3300"><b>'. $usernom->nom.' ' . $smarty->getConfigVars('ajax_Holiday') . ' ' . sqldate2userdateShort($periodeTest->date_debut) . '</b></font>');
				$j++;
			}
...
function checkTechnician($lieu_id, $date_debut, $matin, $apresmidi, $nuit, $date_fin, $periode_id) {
	...
	for($i = 1; $i <= $ndays; $i++){
		...
		$date_aux = calculerDateFin($date_debut,$i);
		$sql = 'SELECT * FROM planning_periode WHERE lieu_id =' . val2sql($lieu_id) . 'AND date_debut = ' . val2sql($date_aux) . 'AND duree_details IN (' . val2sql($details1) . ','. val2sql($details2) .','. val2sql($details3) . ','. val2sql($details4) . ')';
		if($periode_id != '') {
			$sql .= ' AND periode_id <> ' . val2sql($periode_id);
		}
		...
		if($periodeTest->getCount() >= 4) {
			$j++;
			$objResponse->addAssign('divStatutCheckTechnician', 'innerHTML', '<font color="#FF3300"><b>' . $smarty->getConfigVars('ajax_IDTech') . ' </b></font>');
...
function moveCasePeriode($casePeriode, $jourCible, $copie = false, $scope = 'seule') {
    ...
    switch ($jourCible) {
        ...
        case "allDay":
            ...
            if($periode->projet_id == 'holiday')
            {
                $periode->projet_id = 'timetable';
            }
	...
	if($chaines1[2] == $chaines2[2] && $chaines1[3] == $chaines2[3] && $periode->user_id == $chaines2[1] && !($periode->estFilleOuParente() && !$periode->estFilleDerniere()))
	{
		$copie='true';
	} 
	if($copie == 'true') {
		...
		if(isset($projetCible)) {
			...
			if ($periode->user_id == $periode->lieu) 
            {
                $periode->lieu = $userCible->user_id;
            }
		...
		if(CONFIG_PLANNING_ONE_ASSIGNMENT_MAX_PER_DAY == 1) {
			...
			if($periode->isSaved()) {
                $objResponse->addAlert(addslashes('saved'));
...
function submitFormUser($user_id, $user_id_origine, $user_groupe_id, $nom, $email, $login, $password, $visible_planningOui, $couleur, $notificationsOui, $envoiMailPwd, $droits, $adresse, $telephone, $mobile, $metier, $commentaire, $login_actifOui, $specific_users_ids, $ruser_id, $ressource = '') {
	...
	if($visible_planningOui == 'true') {
		...
		$conn = mysqli_connect("localhost", "root",'', "nvh");
		$sql = "SELECT count(*) FROM `planning_user` WHERE `visible_planning`='oui'";
		$result = mysqli_query($conn, $sql);
		$result = mysqli_fetch_assoc($result);
		$number = $result['count(*)'];
		$user_form->order_planning = $number;
	} else {
...
function submitFormPeriode($periode_id, $projet_id, $user_id, $date_debut, $conserver_duree, $date_fin, $nb_jours, $duree, $heure_debut, $heure_fin, $matin, $apresmidi, $nuit , $repetition, 
							$dateFinRepetitionJour, $dateFinRepetitionSemaine, $dateFinRepetitionMois, $nbRepetitionJour, $nbRepetitionSemaine, $nbRepetitionMois, $jourSemaine,$exceptionRepetition, $appliquerATous, 
							$statut_tache, $lieu, $ressource, $livrable, $titre, $notes, $lien, $custom, $notif_email, $holiday=0, $vaca='false', $periode_premiere_1='', $periode_derniere_1='', $price='', $sample='') {
	...
	foreach($user_id as $current_user_id) {
		...
		if(userdate2sqldate($dateFinRepetition) == userdate2sqldate($date_debut)) $repetition = '';
...
function modifRessource($ressource_id=null) {
	...
	if (!$_SESSION['isMobileOrTablet'])
	{
		$objResponse->addScript('jQuery("#date_calibrated").datepicker({ showWeek: true, dateFormat: "'.CONFIG_DATE_DATEPICKER.'" });');
	}
	return $objResponse->getXML();
}


function submitFormRessource($ressource_id, $new_ressource_id, $nom, $ressource_groupe_id, $commentaire, $exclusif, $date_calibrated, $no_calibration) {
	$objResponse = new xajaxResponse();
	$smarty = new MySmarty();
	...
	if($date_calibrated == "" && $no_calibration == 'false'){
		$objResponse->addAlert("Please fill an calibration date or check no apply");
		return $objResponse;
	}
	...
	if($date_calibrated < date("d-m-Y")){
		$ressource->calibrated = 0;
	}
	if($no_calibration == 'true'){
	...
	if(trim($date_calibrated) != '')
	{
		$date_calibrated=forceUserDateFormat($date_calibrated);
	}

	$ressource->nom = $nom;
	$ressource->date_calibrated = ($date_calibrated != '' ? $date_calibrated : null);
	...
	if($ressource_groupe_id == ''){
		$ressource->ressource_groupe_id = 1;
	}
	
	if(!is_null($ressource->date_calibrated)) {
		$ressource->date_calibrated = userdate2sqldate($ressource->date_calibrated);
	}
	
	if($no_calibration == 'true'){
		$ressource->date_calibrated = NULL;
	}
...
function upPlanning($user_id = null){
	$objResponse = new xajaxResponse();
	$smarty = new MySmarty();
	...
	$userDown = new User();
	if($user_id != '') {
		$userUp->db_load(array('user_id', '=', $user_id));
	}

	$position = $userUp->order_planning;
	$userDown->db_load(array('order_planning', '=', $position-1));
	
	$userUp->order_planning = $position-1;
	$userDown->order_planning = $position;
...
function downPlanning($user_id = null){
	$objResponse = new xajaxResponse();
	$smarty = new MySmarty();
	...
	$userDown = new User();
	if($user_id != '') {
		$userDown->db_load(array('user_id', '=', $user_id));
	}

	$position = $userDown->order_planning;
	$userUp->db_load(array('order_planning', '=', $position+1));
	
	$userDown->order_planning = $position+1;
	$userUp->order_planning = $position;
