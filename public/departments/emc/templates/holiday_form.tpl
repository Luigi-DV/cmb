...
<form class="form-horizontal" method="POST" target="_blank" id="periodForm">
	...
	<div class="form-group row col-md-12">
		...
		<div class="col-md-10">
				<select multiple="multiple" name="user_id" id="user_id" class="form-control {if $smarty.session.isMobileOrTablet!=1}select2{/if}" tabindex="2" style="width:100%">
					...
					{foreach from=$listeUsers item=userCourant name=loopUsers}
						...
						{/if}
						<option value="{$userCourant.user_id}" {if $periode.user_id eq $userCourant.user_id}selected="selected"{/if} {if isset($ressource_id) && $ressource_id eq $userCourant.user_id}selected="selected"{/if}>{$userCourant.nom}</option>
						{if $userCourant.user_groupe_id neq $groupeTemp}
							</optgroup>
						{/if}
		...
		<div class='hidden col-md-12'><hr /></div>
		{*Fila en la cual aparece fecha de inicio, por defecto es aquella del dia sobre el cual se selecciona*}
		...
		<div class="form-group row col-md-12">
			...
			<div class="col-md-2">
				{if $smarty.session.isMobileOrTablet==1}
					<input type="date" class="form-control" name="date_debut" id="date_debut" maxlength="10" value="{$periode.date_debut|forceISODateFormat}" tabindex="4"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());"/>
				{else}
					<input type="text" class="form-control datepicker" name="date_debut" id="date_debut" maxlength="10" value="{$periode.date_debut|sqldate2userdate}" tabindex="4"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());"/>
				{/if}
			</div>
			<label class="col-md-1 col-form-label">To :</label>
			<div class="col-md-2">
			{if $smarty.session.isMobileOrTablet==1}
				<input type="date" class="form-control" id="dateFinRepetitionJour" value="" size="11" maxlength="10" onFocus="remplirDateRepetition(this.id);" tabindex="18"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());">
			{else}
				<input type="text" class="form-control datepicker" id="dateFinRepetitionJour" value="" size="11" maxlength="10" onFocus="remplirDateRepetition(this.id);" tabindex="18"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());">
			{/if}
			</div>
		</div>
		...
		<div class="form-group row col-md-12">
			...
			<div class="col-md-8 pt-2">	
					...
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="checkbox" name="allday" id="allday" onChange="videChampsFinTache(this.id);" {if $periode.duree_details eq 'duree' && $periode.projet_id eq 'timetable'}checked="checked"{/if}>
					...
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="checkbox" name="holiday" id="holiday" onChange="videChampsFinTache(this.id);" {if $periode.duree_details eq 'duree' && $periode.projet_id eq 'holiday'}checked="checked"{/if}>
						<label class="form-check-label" for="holiday">{#winPeriode_holiday#}</label>
					</div>
			</div>
		</div>
...
{* TO HIDE solo se quiere dejar la opción de mañana o tarde*}
		...
		{if $smarty.const.CONFIG_SOPLANNING_OPTION_LIEUX == 1 }
			<label class="col-md-2 col-form-label">{#winPeriode_lieu#} :</label>
				<div class="col-md-4">
					<select name="lieu" id="lieu" class="form-control {if $smarty.session.isMobileOrTablet!=1}select2{/if}" tabindex="22" style="width:100%" >
						<option value=""></option>
						{foreach from=$listeLieux item=lieuTmp}
							<option value="{$lieuTmp.lieu_id}" {if $periode.lieu eq $lieuTmp.lieu_id}selected="selected"{/if} {if isset($user_id_choisi) && $user_id_choisi eq $lieuTmp.lieu_id}selected="selected"{/if}>{$lieuTmp.nom}</option>
						{/foreach}
					</select>
				</div>
		{/if}
		...
		<div class="form-group row col-md-12">
			<div class="form-check form-check-inline">
			<div class="form-check form-check-inline">
				<input class="form-check-input" type="checkbox" id="appliquerATous" value="1"><label class="form-check-label" for="appliquerATous">{#winPeriode_appliquerATous#}</label>
			</div>
			</div>
		</div>
		<hr />
		<div class="form-group row col-md-12">
			<div class="form-check form-check-inline">
				<input class="form-check-input" type="checkbox" id="applicationToSomeone" value="1"><label class="form-check-label" for="applicationToSomeone">Apply modification to ocurrences: </label>
			</div>
			<label class="col-md-2 col-form-label">From :</label>
			<div class="col-md-2">
				<input type="text" class="form-control datepicker" id="date_init" maxlength="10" tabindex="4" value="{$periode.date_debut|sqldate2userdate}" onFocus="remplirDateRepetition(this.id);" onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());" readonly/>
			</div>
			<label class="col-md-1 col-form-label">To :</label>
			<div class="col-md-2">
				<input type="text" class="form-control datepicker" id="date_final" value="" size="11" maxlength="10" tabindex="18" onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());">
			</div>
		</div>
		<hr />
		{/if}
			<div class="btn-group" role="group">
				...
				{if $periode.periode_id neq 0}
					...
					<button type="button" class="hidden btn btn-default" onClick="if(confirm('{#winPeriode_dupliquer#|xss_protect} ?'))xajax_ajoutHoliday('', '', {$periode.periode_id});">{#winPeriode_dupliquer#}</button>
					{if isset($estFilleOuParente)}
					...
					{/if}
				{/if}
				<button type="button" id="butSubmitPeriode" class="btn btn-primary" tabindex="24" onClick="$('#divPatienter').removeClass('d-none');this.disabled=true;users_ids=getSelectValue('user_id');ressources_ids=getSelectValue('ressource');
				xajax_submitFormPeriode('{$periode.periode_id}', 'timetable', users_ids, $('#date_debut').val(), $('#conserver_duree').is(':checked'), $('#date_fin').val(), $('#nb_jours').val(), $('#duree').val(), $('#heure_debut').val(), $('#heure_fin').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'), $('#nuit').is(':checked'), $('#repetition option:selected').val(), $('#dateFinRepetitionJour').val(),$('#dateFinRepetitionSemaine').val(),$('#dateFinRepetitionMois').val(), $('#nbRepetitionJour option:selected').val(),$('#nbRepetitionSemaine option:selected').val(),$('#nbRepetitionMois option:selected').val(),getRadioValue('jourSemaine'),getRadioValue('exceptionRepetition'),$('#appliquerATous').is(':checked'), $('#applicationToSomeone').is(':checked'), $('#date_init').val(), $('#date_final').val(), $('#statut_tache').val(),$('#lieu option:selected').val(), ressources_ids, $('#livrable').val(), $('#titre').val(), $('#notes').val(), $('#lien').val(), $('#custom').val(), $('#notif_email').is(':checked'), 1, $('#holiday').is(':checked'),$('#periode_premiere').val(), $('#periode_derniere').val(), '', '', '', '');">{#winPeriode_valider#|xss_protect}</button>
			</div>
