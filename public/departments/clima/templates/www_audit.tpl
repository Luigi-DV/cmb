...
<div class="container">
	{if $smarty.const.CONFIG_SOPLANNING_OPTION_AUDIT == 1 && in_array("audit_restore", $user.tabDroits) && $user.user_id == 'ADM'}
	<div class="row">
		<div class="col-md-12">
					...
					<table class="table table-striped table-hover" id="traceTab">
						...
						{foreach name=audits item=audits from=$audit}
							<tr>
								...
								{if ($audits.type=="MT" or $audits.type=="DT" or $audits.type=="MP" or $audits.type=="DP" or $audits.type=="MG" or $audits.type=="DG" or $audits.type=="MU" or $audits.type=="DU" or $audits.type=="ME" or $audits.type=="DE" or $audits.type=="ML" or $audits.type=="DL" or $audits.type=="MR" or $audits.type=="DR" or $audits.type=="MS" or $audits.type=="DS" or $audits.type=="SD" or $audits.type=="SM") and $audits.nbmodifs > 0}
								...
								<td class="wrap">
									...
									{if $audits.type=="SA"}{#action_ajout_sample#}"{$audits.informations}"{/if}		
									{if $audits.type=="SM"}{#action_modif_sample#} "{$audits.informations}"{/if}
									{if $audits.type=="SD"}{#action_suppression_sample#} "{$audits.informations}"{/if}
									&nbsp;
								</td>
...
$("#filtreUserAudit").multiselect({
	...
	placeholder: '{/literal}<i class="fa fa-user fa-lg fa-fw" aria-hidden="true"></i><span class="d-none d-md-inline-block">&nbsp;{#formChoixUserAudit#}</span>{literal}',
	texts: {
