...
{
	$sql .= " ORDER BY pu.order_planning";
...
$sql = "SELECT planning_periode.*,planning_projet.statut, planning_status.nom as status_nom,  planning_status.barre as statut_barre,planning_status.gras as statut_gras,planning_status.italique as statut_italique,planning_status.souligne as statut_souligne, planning_status.couleur as statut_couleur,planning_status.pourcentage as statut_pourcentage, pu.nom as user_nom, pu.couleur as user_couleur,
		...
		puc.nom AS nom_createur, pum.nom AS nom_modifier, pm.nom AS test_manager,
		CASE 
...
if(count($_SESSION['filtreGroupeLieu']) > 0) {
	//Per filtar tasques amb més d'un treballador
	$mysqli = new mysqli("localhost", "root",'', "nvh");
	$res = $mysqli->query("SELECT DISTINCT lieu_id FROM `planning_periode` WHERE lieu_id!='NULL'");
	for ($num_fila = $res->num_rows - 1; $num_fila >= 0; $num_fila--) {
		$fila = $res->fetch_assoc();
		if(strlen($fila['lieu_id'])>3){
			$lieus = preg_split('/,/', $fila['lieu_id'],-1, PREG_SPLIT_NO_EMPTY);
			foreach ($lieus as $l){
				foreach ($_SESSION['filtreGroupeLieu'] as $s){
					if($l == $s){
						array_push($_SESSION['filtreGroupeLieu'], $fila['lieu_id']);
					}
				}
			}
		}
...
while ($p = $periodes->fetch()) {
	...
	if (!in_array($infosJour['periode_id'],$planning['periodes']))
	{
		...
		$cellule=array(
			...
			'test_manager'=>$infosJour['test_manager'],
		...
		foreach ($planning['colonnes'] as $cle_colonne) 
		{
			// Planning Jour ou User
				...
				if (isset($joursOccupes[$current_date])) 
				{
					...
					if(strpos($html_aux, "cellProjectAM") && !strpos($html_aux, "cellProjectPM") && !strpos($html_aux, "cellProject ") && !strpos($html_aux, "cellProjectcons ")) {
						$html.= $html_aux;
						$html.= '<div class="cellLine"></div>';
						$html.= '<div id="td_' . $ligneId . '_' . $current_date2 . '_PM" '. $styleTD. ' class="cellTask' . ($dimensionCase == 'large' ? '' : 'cons') . '" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);"></div>';
					} 
					
					//solo tareas de tardes
					elseif (!strpos($html_aux, "cellProjectAM") && strpos($html_aux, "cellProjectPM") && !strpos($html_aux, "cellProject ") && !strpos($html_aux, "cellProjectcons ")) {
						$html.= '<div id="td_' . $ligneId . '_' . $current_date2 . '_AM" '. $styleTD. ' class="cellTask' . ($dimensionCase == 'large' ? '' : 'cons') . '" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);"></div>';
						$html.= '<div class="cellLine"></div>';
				...
				} else 
				{
					...
					$html.= '<div id="td_' . $ligneId . '_' . $current_date2 . '_' . 'AM" '. $styleTD. ' class="cellTask' . ($dimensionCase == 'large' ? '' : 'cons') . '" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);"></div>';
					$html.= '<div class="cellLine"></div>';
					$html.= '<div id="td_' . $ligneId . '_' . $current_date2 . '_PM" '. $styleTD. ' class="cellTask' . ($dimensionCase == 'large' ? '' : 'cons') . '" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="leaveDropZone(event);"></div>';
					$html.= '<div class="cellLine"></div>';
