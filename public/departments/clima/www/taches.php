...
if(isset($_REQUEST['rechercheTaches']) && $_REQUEST['rechercheTaches'] != ''){
	...
	$sql.= " WHERE (" . $isLike . ") AND planning_projet.statut in ('" . implode("','", $listeStatuts) . "') AND planning_periode.projet_id != 'timetable' AND planning_periode.projet_id != 'holiday'";
	...
	$sql .= "ORDER BY nom_groupe ASC, $order ASC," . $order . ' ' . $by;
	$smarty->assign('rechercheTaches', $_REQUEST['rechercheTaches']);

}  else {
	// recuperation des projets couvrant la période
	$sql = "select planning_periode.*, planning_projet.*, planning_groupe.nom AS nom_groupe, planning_user2.nom AS nom_personne , planning_user.nom AS nom_createur, planning_periode.lien as lien, ps.nom as status_nom, ps.couleur as statut_couleur  
			FROM planning_periode
			LEFT JOIN planning_projet ON planning_projet.projet_id = planning_periode.projet_id
			LEFT JOIN planning_groupe ON planning_groupe.groupe_id = planning_projet.groupe_id
			LEFT JOIN planning_user ON planning_user.user_id = planning_projet.createur_id 
	...
	$sql .= " WHERE planning_periode.statut_tache in ('" . implode("','", $listeStatuts) . "') 
			AND planning_periode.projet_id != 'timetable' AND planning_periode.projet_id != 'holiday'
			AND (
	...
	{
	 $sql .=" ORDER BY nom_groupe ASC, planning_periode.projet_id ASC," . $order . ' ' . $by. ",planning_periode.statut_tache ASC";
	}elseif ($grouperpar=='status') 
	{
		$sql .=" ORDER BY nom_groupe ASC, planning_periode.statut_tache ASC," . $order . ' ' . $by.",planning_periode.projet_id ASC";
	}elseif ($grouperpar=='utilisateur') 
	{
		$sql .=" ORDER BY nom_groupe ASC, nom_personne ASC," . $order . ' ' . $by.",planning_periode.projet_id ASC";
	}
...
$sql .= "WHERE pp.projet_id != 'holiday' AND pp.projet_id != 'timetable' AND
			(
