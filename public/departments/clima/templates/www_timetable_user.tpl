...
{include file="www_timetable_filtre_user.tpl"}
	{* le planning *}
	<div class="row">
...
{literal}
	...
	// Gestion du filtre Projet
		$("#filtreGroupeProjet").multiselect({
			...
			nameSuffix: 'projet',
			desactivateUrl: 'process/timetable_user.php?desactiverFiltreGroupeProjet=1',
	...
	// Gestion du filtre Resource
		$("#filtreUser").multiselect({
			selectAll:false,
			noUpdatePlaceholderText:true,
			nameSuffix: 'user',
			desactivateUrl: 'process/timetable_user.php?desactiverFiltreUser=1',
	...
	// Gestion du filtre Users
		$("#filtreGroupeLieu2").multiselect({
			selectAll:false,
			noUpdatePlaceholderText:true,
			nameSuffix: 'ressource',
			desactivateUrl: 'process/timetable_user.php?desactiverFiltreLieu=1',
	...
	$('#tabContenuPlanning td.week,#tabContenuPlanning td.weekend,#tabContenuPlanning .cellProject,#tabContenuPlanning .cellProjectAM,#tabContenuPlanning .cellProjectPM,#tabContenuPlanning .cellProjectN').click(function(ev){
	ev.preventDefault();
	if ($(this).hasClass("cellProject") || $(this).hasClass("cellProjectAM") || $(this).hasClass("cellProjectPM") || $(this).hasClass("cellProjectN"))
		{
		 cellClic2(this.id,0);
		}else cellClic2(this.id,1);
		return false;
	});
	{/literal}
	//COCACOLITA
