...
<form class="form-horizontal" method="POST" target="_blank" id="periodForm">
	...
	<div class="form-group row col-md-12">
		{*Fila en la cual se escoje el proyecto asociado a la rerserva, se selecciona a partir de los elementos de la base de datos*}
			...
			<label hidden class="col-md-2 col-form-label">{#winPeriode_user#}:</label>
		...
		<div hidden class='col-md-12'><hr /></div>
		<div hidden class="form-group row col-md-12">
		...
		<div hidden class="form-group row col-md-12">
			...
			<div class="col-md-2">
				{if $smarty.session.isMobileOrTablet==1}
					<input type="date" class="form-control" name="date_debut" id="date_debut" maxlength="10" value="{$periode.date_debut|forceISODateFormat}" tabindex="4"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());xajax_checkHoliday($('#lieu option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');xajax_checkRessource(getSelectValue('ressource'), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');">
				{else}
					<input type="text" class="form-control datepicker" name="date_debut" id="date_debut" maxlength="10" value="{$periode.date_debut|sqldate2userdate}" tabindex="4"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());xajax_checkHoliday($('#lieu option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');xajax_checkRessource(getSelectValue('ressource'), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');">
				{/if}
			</div>
			...
			<div class="col-md-2">
				{if $smarty.session.isMobileOrTablet==1}
					<input type="date" class="form-control" id="dateFinRepetitionJour" value="" size="11" maxlength="10" onFocus="remplirDateRepetition(this.id);" tabindex="18"  onChange="xajax_checkTaskDate($('#projet_id').val(), $('#date_debut').val(), $('#dateFinRepetitionJour').val());xajax_checkHoliday($('#lieu option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');xajax_checkRessource(getSelectValue('ressource'), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');">
				{else}
...
{* TO HIDE solo se quiere dejar la opción de mañana o tarde*}
		...
		{if $smarty.const.CONFIG_SOPLANNING_OPTION_LIEUX == 1 }
			<div class="col-md-2 col-form-label">{#winPeriode_lieu#} IND:</div>
				<div class="col-md-3">
					<select multiple name="lieu_AUTO" id="lieu_id_IND" class="form-control {if $smarty.session.isMobileOrTablet!=1}select2{/if}" "tabindex="22" style="width:100%" onChange="xajax_checkHoliday($('#lieu_id option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');xajax_checkTechnician($('#lieu_id option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');">
						{foreach from=$listeLieuxIND item=lieuTmp_IND}
							<option value="{$lieuTmp_IND.lieu_id}" {if in_array($lieuTmp_IND.lieu_id, $periode.lieu)} selected="selected" {/if}>{$lieuTmp_IND.nom}</option>
						{/foreach}
					</select>
					<span id="divStatutCheckTechnician"></span>
					<span id="divStatutCheckHoliday"></span>
				</div>
		{/if}
		{if $smarty.const.CONFIG_SOPLANNING_OPTION_LIEUX == 1 }
			<div class="col-md-2.5 col-form-label" style="padding-left: 25px;padding-right: 10px;">{#winPeriode_lieu#} AUTO:</div>
				<div class="col-md-3">
					<select multiple name="lieu_AUTO" id="lieu_id_AUTO" class="form-control {if $smarty.session.isMobileOrTablet!=1}select2{/if}" tabindex="22" style="width:100%" onChange="xajax_checkHoliday($('#lieu_id option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');xajax_checkTechnician($('#lieu_id option:selected').val(), $('#date_debut').val(), $('#matin').is(':checked'), $('#apresmidi').is(':checked'),$('#nuit').is(':checked'), $('#dateFinRepetitionJour').val(), '{$periode.periode_id}');">
						<option value=""></option>
						{foreach from=$listeLieuxAUTO item=lieuTmp_AUTO}
							<option value="{$lieuTmp_AUTO.lieu_id}" {if in_array($lieuTmp_AUTO.lieu_id, $periode.lieu)} selected="selected" {/if}>{$lieuTmp_AUTO.nom}</option>
						{/foreach}
					</select>
					<span id="divStatutCheckTechnician"></span>
					<span id="divStatutCheckHoliday"></span>
		...
		<div hidden class="row col-md-12">
			<div class="form-check form-check-inline">
					<input class="form-check-input" type="checkbox" name="client" id="client" {if $periode.client == '1'}checked="checked"{/if}>
					<label class="form-check-label" for="client"> Presencia Client </label>
			</div>
		</div>
		<div hidden class="row col-md-12">
			<div class="form-check form-check-inline">
					<input class="form-check-input" type="checkbox" name="reservaHP" id="reservaHP" {if $periode.reserva_HP == '1'}checked="checked"{/if}>
					<label class="form-check-label" for="reservaHP"> Probabilitat Reserva HP </label>
			</div>
		</div>
		...
		{/if}
			<div class="btn-group" role="group">
				...
				{if $periode.periode_id neq 0}
					...
					{/if}
				{/if}
				<button hidden type="button" id="butSubmitPeriode" class="btn btn-primary" tabindex="24" onClick="
					$('#divPatienter').removeClass('d-none');
					this.disabled=true;
					users_ids=getSelectValue('user_id');
					lieus_ids = getTwoSelectValues('lieu_id_IND','lieu_id_AUTO');
					...
					xajax_submitFormPeriode('{$periode.periode_id}', $('#projet_id').val(), users_ids, $('#date_debut').val(), $('#conserver_duree').is(':checked'), $('#date_fin').val(), $('#nb_jours').val(), $('#duree').val(), $('#heure_debut').val(), $('#heure_fin').val(), 
											...
											ressources_ids, $('#livrable').val(), $('#titre').val(), $('#notes').val(), $('#lien').val(), $('#custom').val(), $('#notif_email').is(':checked'), 0, 'false', $('#periode_premiere').val(), $('#periode_derniere').val(), $('#price').val(), 
											$('#reservaHP').is(':checked'), $('#sample').is(':checked'), $('#client').is(':checked'));
					">{#winPeriode_valider#|xss_protect}</button>
				<button type="button" id="butSubmitMultiplePeriode" class="btn btn-primary" tabindex="24" onClick="
					$('#divPatienter').removeClass('d-none');
					this.disabled=true;
					users_ids=getSelectValue('user_id');
					lieus_ids = getTwoSelectValues('lieu_id_IND','lieu_id_AUTO');
					...
					{foreach from=$data item=$d}
						users[0] = {$d.user_id};
						xajax_submitFormPeriode('{$periode.periode_id}', $('#projet_id').val(), users, '{$d.datedebut|sqldate2userdate}', $('#conserver_duree').is(':checked'), $('#date_fin').val(), $('#nb_jours').val(), $('#duree').val(), $('#heure_debut').val(), $('#heure_fin').val(), 
											'{if $d.div == 'AM'}true{else}false{/if}', '{if $d.div == 'PM'}true{else}false{/if}', '{if $d.div == 'N'}true{else}false{/if}', 'jour', $('#dateFinRepetitionJour').val(),$('#dateFinRepetitionSemaine').val(),$('#dateFinRepetitionMois').val(), $('#nbRepetitionJour option:selected').val(),
											$('#nbRepetitionSemaine option:selected').val(),$('#nbRepetitionMois option:selected').val(),getRadioValue('jourSemaine'),getRadioValue('exceptionRepetition'),$('#appliquerATous').is(':checked'), '', '', '', $('#statut_tache').val(),lieus_ids, 
											ressources_ids, $('#livrable').val(), $('#titre').val(), $('#notes').val(), $('#lien').val(), $('#custom').val(), $('#notif_email').is(':checked'), 0, 'false', $('#periode_premiere').val(), $('#periode_derniere').val(), $('#price').val(), 
											$('#reservaHP').is(':checked'), $('#sample').is(':checked'), $('#client').is(':checked'));
					{/foreach}
